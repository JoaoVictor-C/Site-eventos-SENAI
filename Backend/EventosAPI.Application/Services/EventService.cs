...
namespace EventosAPI.Application.Services
{
    public class EventService : IEventService
    {
        ...
        public async Task<EventRoleDto> AssignRoleAsync(AssignEventRoleDto roleDto)
        {
            var @event = await _eventRepository.GetByIdAsync(roleDto.EventId)
                ?? throw new NotFoundException(nameof(Event), roleDto.EventId);
            ...
            if (!eventRole.Validate(out var errors))
                throw new ValidationException(errors);

            await _eventRepository.AddRoleAsync(eventRole);
            return _mapper.Map<EventRoleDto>(eventRole);
        }
        ...
        public async Task RemoveRoleAsync(Guid eventId, Guid userId, EventRoleType roleType)
        {
            ...
            if (!await HasEventRoleAsync(eventId, userId, roleType))
                throw new NotFoundException($"Role {roleType}", $"Event: {eventId}, User: {userId}");

            await _eventRepository.RemoveRoleAsync(eventId, userId, roleType);
        }

        public async Task<IEnumerable<EventRoleDto>> GetEventRolesAsync(Guid eventId, Guid userId)
        {
            var @event = await _eventRepository.GetByIdAsync(eventId)
                ?? throw new NotFoundException(nameof(Event), eventId);

            var allRoles = await _eventRepository.GetUserEventRolesAsync(userId);
            var rolesForEvent = allRoles.Where(r => r.EventId == eventId);
            return _mapper.Map<IEnumerable<EventRoleDto>>(rolesForEvent);
        }
        ...
        public async Task<IEnumerable<EventRoleDto>> GetUserEventRolesAsync(Guid userId)
        {
            ...
            var roles = await _eventRepository.GetUserEventRolesAsync(userId);
