...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      // Optionally, fetch user profile and set auth context
  ...
  //   window.location.href = `${'http://localhost:5027'}/api/v1/auth/google/login?returnUrl=${encodeURIComponent(window.location.origin + '/login')}`;
  // };
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8 space-y-8">
          ...
          <div className="text-center">
            <Text variant="small" className="text-gray-600 dark:text-gray-400">
              Não tem uma conta?
              <Link to={ROUTES.AUTH.REGISTER} className="text-senai-red hover:underline ml-1">Cadastre-se</Link>
            </Text>
          </div>
        </div>
      </motion.div>
